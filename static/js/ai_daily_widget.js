(() => {
  ...
  function installAiLedgerAssets() {
    if (!document.getElementById('ai-pick-ledger-v52-style')) {
      const link = document.createElement('link'); link.id='ai-pick-ledger-v52-style'; link.rel='stylesheet'; link.href='/static/css/ai_pick_ledger_v52.css?v=20260917v57'; document.head.appendChild(link);
    }
    if (!document.getElementById('ai-pick-ledger-v52-script')) {
      const script=document.createElement('script'); script.id='ai-pick-ledger-v52-script'; script.src='/static/js/ai_pick_ledger_v52.js?v=20260917v57'; script.async=false; document.body.appendChild(script);
    }
  }
  function addStyle(){
  ...
  async function mount(attempt=0){const s=placeSection();if(!s){if(attempt<80)setTimeout(()=>mount(attempt+1),100);return}addStyle();try{const d=await loadDay();s.innerHTML=cardMarkup(d)}catch(e){s.innerHTML='<span style="color:#8b95a1;font-size:13px">PICK 데이터를 불러오지 못했습니다.</span>'}}
