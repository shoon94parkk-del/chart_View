...
(() => {
  ...
  function provenance(stock, key, quoteRow, generatedAt) {
    ...
    if ((key === 'forwardPE' || key === 'forwardEPS') && !present(stock[key])) {
      return `12M 전망 · 컨센서스 데이터 없음`;
    }
    if (key === 'forwardPE' || key === 'forwardEPS') {
      return `12M 전망 · ${today()} 조회 · Yahoo QuoteSummary`;
    }
    if (key === 'price') return `${today()} 조회 · Yahoo Chart`;
    if ((key === 'trailingPE' || key === 'pbr' || key === 'dividendYield') && isKR && sourceText.includes('Naver')) {
      return `${today()} 조회 · Naver Finance`;
    }
    if (key === 'trailingPE' || key === 'trailingEPS' || key === 'psr') {
      return `TTM · ${today()} 조회 · Yahoo Fundamentals`;
    }
    if (key === 'pbr' || key === 'bookValue') {
      return `최근 공시 · ${today()} 조회 · Yahoo Fundamentals`;
    }
    if (key === 'evEbitda' || key === 'roe' || key === 'operatingMargin') {
      return `TTM/최근 공시 · ${today()} 조회 · Yahoo Fundamentals`;
    }
  ...
  async function annotateTable() {
    ...
    table.querySelectorAll('tbody tr').forEach((row) => {
      ...
      Array.from(row.children).forEach((cell, index) => {
        ...
        let meta = cell.querySelector('.metric-provenance');
        if (!meta) {
  ...
  function init() {
    ...
    document.querySelectorAll('.metric-chip, .sort-btn').forEach((el) => {
      el.addEventListener('click', () => setTimeout(annotateTable, 0));
    });
  }
