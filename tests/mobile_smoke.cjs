...
(async () => {
  ...
  try {
    for (const width of [360, 384, 430]) {
      ...
      const visibleHomeWatch = await page.locator('#home-watchlist-v30 [data-home-watch-open]:visible').count();
      assert(visibleHomeWatch <= 4, `${width}: Home watchlist should show at most four cards, got ${visibleHomeWatch}`);
      await page.locator('#ai-daily-section').waitFor({timeout:30000});
      await page.locator('#ai-daily-section .ai-daily-price').first().getByText(/추천 .* 종가 .*→ 현재/).waitFor({timeout:30000});
      assert.equal(await page.locator('#ai-daily-section .ai-daily-rank').count(), 0, `${width}: PICK rank labels must be hidden`);
