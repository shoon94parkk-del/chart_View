"""Generate a free macro-data cache for the Render app.

FRED is fetched from GitHub Actions rather than from Render shared IPs, which can
be slow or blocked. The web app then serves this JSON instantly.
"""
...
INDICATORS: dict[str, dict[str, str | None]] = {
    ...
    "FEDFUNDS": {"name": "연방기금금리 (Fed Rate)", "desc": "미국 유효 연방기금금리의 월간 시계열입니다.", "link": "https://fred.stlouisfed.org/series/FEDFUNDS", "source": "FRED"},
    "^VIX": {"name": "공포 지수 (VIX)", "desc": "미국 주식시장의 기대 변동성을 나타냅니다.", "link": "https://finance.yahoo.com/quote/%5EVIX", "source": "YAHOO"},
}


def fetch_fred(symbol: str, meta: dict[str, Any]) -> dict[str, Any]:
...
def fetch_vix(meta: dict[str, Any]) -> dict[str, Any]:
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=6) as pool:
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(symbol, results[symbol]["value"], results[symbol]["asOf"])
    ...
    print(f"wrote {len(ordered)} macro series -> {OUT}")
