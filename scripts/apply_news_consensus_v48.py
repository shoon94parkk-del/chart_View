...
NEWS_FALLBACK = r'''def _finance_korean_fallback(text: str, title: str = "") -> str:
...
CONSENSUS_FETCH = '''def fetch_consensus(symbol: str) -> dict[str, Any]:
...
def main() -> None:
    news_path = Path("news_summary_service.py")
    news = news_path.read_text(encoding="utf-8")
    news = news.replace('SUMMARY_CACHE_VERSION = "v46-ko-resilient"', 'SUMMARY_CACHE_VERSION = "v48-fact-first"')
    news = splice(news, "def _finance_korean_fallback", "\ndef _trim_summary", NEWS_FALLBACK)
    news_path.write_text(news, encoding="utf-8")

    consensus_path = Path("consensus_service.py")
    cons = consensus_path.read_text(encoding="utf-8")
    if "DISK_FRESH_AGE" not in cons:
        cons = cons.replace(
            "DISK_MAX_AGE = 36 * 3600",
            "DISK_FRESH_AGE = 36 * 3600\nDISK_MAX_AGE = 7 * 24 * 3600",
        )
    start = cons.index("def fetch_consensus(symbol: str) -> dict[str, Any]:")
    cons = cons[:start] + CONSENSUS_FETCH.rstrip() + "\n"
    consensus_path.write_text(cons, encoding="utf-8")
