...
**Decision:** add a random browser heartbeat and store only salted daily hashes. Current activity remains in process memory for cheap worker gating, while the daily unique-browser set is persisted in the free Render Key Value cache with memory fallback. The private `/admin/usage` page uses a server environment token and is not linked from the public UI. The metric is an approximate browser count, not an exact person count.
